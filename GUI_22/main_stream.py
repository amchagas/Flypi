...
@streaming features by isobianin
"""
...
class allcallbacks(Ui_MainWindow):
    ...
    def __init__(self,dialog):
        ...
        self.streambutton.clicked.connect(self.streambutton_callback)
        self.recordbutton.clicked.connect(self.recordbutton_callback)
    ...
    def videobutton_callback(self):
        ...
        t2 = threading.Thread(target = self.run_video)
        t2.start()
    ...
    # Using threading so start_recording() does not freeze the GUI
    # Very easy yet extremely effective way to this
    # https://stackoverflow.com/questions/46057654/
    # threading-a-function-pyqt-gui
    # To disable it (not recommended), rename run_*** method to 
    # the original name and delete the threading method.
    def streambutton_callback(self):
        
        t1 = threading.Thread(target = self.run_stream(self.cam))
        t1.start()
     
    def run_stream(self,Camera):
        
        flag = 1
        if self.streambutton.isChecked() and flag==1:
            flag=0
            ip_inp = self.ipinput.text()
            port_inp = self.portinput.text()
            print ("connecting to server " + ip_inp + ":" + port_inp)
            if self.camFlag==1:
                # Client side code is taken from the official PiCamera
                # documentation https://picamera.readthedocs.io/en/
                # release-1.13/recipes2.html#rapid-capture-and-processing
                # Server side is derived from stackoverflow 
                # https://stackoverflow.com/questions/46624449/
                # load-bytesio-image-with-opencv
                # https://stackoverflow.com/questions/48611517/
                # raspberry-pi-3-python-and-opencv-face-recognition-from-network-camera-stream/48675107#48675107
                try:    
                    self.client_socket = socket.socket()
                    self.client_socket.settimeout(2)
                    self.client_socket.connect((ip_inp, int(port_inp)))
                    # 'settimeout' and 'None' are derived from this. 'None' is
                    # is needed to set the socket into blocking mode
                    # which is needed for 'makefile'
                    # https://stackoverflow.com/questions/3432102/
                    # python-socket-connection-timeout
                    self.client_socket.settimeout(None)
                    self.connection = self.client_socket.makefile('wb')
                    print ("connected to server")
                    self.output = SplitFrames(self.connection)
                    # Allow camera to warm up / adjust before recording
                    time.sleep(2)
                    self.start = time.time()
                    # Quite an interesting solution to have both stream and record.
                    # Record is tied to port=1 of the camera by default
                    # while the stream is manually tied to port=0
                    # The solution derived from the official PiCamera
                    # documentation https://picamera.readthedocs.io/en/
                    # release-1.13/api_camera.html
                    self.cam.start_recording(self.output, format='mjpeg', splitter_port=0)
                    # Utilizing 'streamon' flag akin to 'camon' here it enables 'timeout'
                    # to happen properly without stopping the camera. Previously, it was just
                    # 'start_recording' and then 'stop_recording' when button is pressed again.
                    # Also, it is important to note that 'streamon = True' does not work
                    # properly while 'streamon = 1' or 'streamon = 0' works fine
                    self.streamon = 1 
                    # 'While loop' is no longer needed because while the button
                    # is pressed or 'isChecked()' is 'True', the code present here
                    # will be executed in a regular sequence. However, when the program
                    # reaches this very point (after record), it will run infinitely
                    # We could say that there is a 'while True:' statement unless
                    # we press the button again. It has to be 'while True:' as this is
                    # the only way how 'start_recording' may work without 'wait_recording' or
                    # 'stop_recording'
                    # Incidentally, this was discovered by accident because
                    # 'while loop' was commented
                    # Added wait_recording so it may raise exception if needed. It is
                    # believed that this is the right way to do it
                    self.cam.wait_recording(splitter_port=0) # It is essential to specify port
                # Socket timeout is implemented so there is no exception raised by
                # python itself. This way, if there is no connection to server,
                # this code will simply exit from 'if' statement and enter else
                # statement. Therefore, it will close sockets and do everything
                # nice and properly. Otherwise, it crashes program
                # https://stackoverflow.com/questions/11865685/
                # handling-a-timeout-error-in-python-sockets
                except socket.timeout:
                    print("Unnable to connect to server. Shutting down client")
                    # At first, a custom signal was sent here. But it just
                    # happened that implementing a simple 'click' on the button
                    # if connection fails is efficient and convenient.
                    # However, it was hard to find the right command to click a button
                    # from the code. On stackoverflow it is called a 'programmatic click'
                    # https://stackoverflow.com/questions/21672957/
                    # how-to-create-click-of-button-in-pyqt
                    self.streambutton.click()
                    return
                    # The 'return' above ensures that if an exception occurs,
                    # it will exit to 'else' statement
                return
            ...
            print("stream is off")
            if self.camFlag==1:
                # This line disables camera thus enabling stream to be
                # turned on/off
                if self.streamon == 1:
                    try:
                        # This 'try-except' statement debugs all known issues when
                        # server is closed or has crashed while stream is on.
                        # Two lines below are in 'try-except' as they raise respective exceptions 
                        # [Errno 104] Connection reset by peer and [Errno 32] Broken pipe.
                        # stop_record is here because Errno 104 reacts first of all to
                        # to stop_record and raise exception. Then, logically when server
                        # is down and client sends struct.pack it does not reach server, thus
                        # raising Errno 32.
                        # The name of error is taken from here
                        # https://stackoverflow.com/questions/33111556/
                        # connection-reset-by-peer-errno-104-in-python-2-7
                        # Also, difference between exception from Python 2.x and 3.x
                        # https://stackoverflow.com/questions/4690600/
                        # python-exception-message-capturing
                        # After some more testing of server shutdowns, it appears that
                        # sometimes we still get Errno 32 but this one occurs due to
                        # connection.close(). That is why it is also placed in try-except
                        self.cam.stop_recording(splitter_port=0) # It is essential to specify port
                        self.connection.write(struct.pack('<L', 0))
                        self.connection.close()
                    except socket.error:
                        print("Server was closed during transmission. Shutting down client")
                    self.streamon = 0
                    ...
                    # It is important to add self.output as otherwise it
                    # does not work. Particularly, output variable deinitiates,
                    # thus it will be undeclared here
                    self.output.count, self.finish-self.start, self.output.count /
                    (self.finish-self.start)))
    # All the functionality is taken straight from the videobutton
    def recordbutton_callback(self):
        
        t3 = threading.Thread(target = self.run_record(self.cam))
        t3.start()
    
    def run_record(self,Camera):
        
        flag = 1
        if self.recordbutton.isChecked() and flag==1:
            flag=0
            print ("recording until stopped")

            if self.camFlag==1:
                self.videoon = 1 # recordon is already taken
                videoPath = self.create_folder(folderPath=self.basePath,
                               folderName='videos/')
            ...
            self.cam.start_recording(output = videoPath +
                                'video_' +
                                time.strftime('%Y-%m-%d-%H-%M-%S') + '.h264',
                                format = "h264", splitter_port = 2) # Different splitter
                                # port for a case if somebody runs record for specific time,
                                # stream and this function
            self.cam.wait_recording(splitter_port = 2) # It is essential to specify port
            

        else:
            self.cam.stop_recording(splitter_port = 2) # It is essential to specify port
            print("done.")
            #here we restore the preview resolution if it was the maximal one.
            if self.resVal == "2592x1944":
                if self.camFlag==1:
